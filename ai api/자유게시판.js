...
router.use('/Free_board', express.static(path.join(__dirname, '게시판모음')));

// 리뷰 게시판 목록 조회 라우터
router.get('/Free_board', (req, res) => {
...
// 글쓰기 페이지 라우트
router.get('/Free_board/post', (req, res) => {
...
router.get('/Free_board/:id', (req, res) => {
  const free_id = req.params.id;


  // 조회수 증가 쿼리
  const increaseViewCountQuery = 'UPDATE free_board SET hits = hits + 1 WHERE num = ?';
  
  connection.query(increaseViewCountQuery, [free_id], (err, result) => {
    if (err) {
      ...
      const query = `
        ...
        FROM
          free_board rb
          LEFT JOIN comments_free cr ON rb.num = cr.free_id
        WHERE
          rb.num = ?
        `;
      connection.query(query, [free_id], (err, rows) => {
        if (err) {
...
// 게시글 등록 라우트
router.post('/Free_board/board/post', (req, res) => {
  ...
  // 데이터베이스에 게시글 정보 저장
  const query = 'INSERT INTO free_board (title, content, member_id, rdate) VALUES (?, ?, ?, ?)';
...
// 게시글 삭제 라우트
router.post('/Free_board/board/delete/:id', (req, res) => {

  //보안
  // Validate inputs
  req.check('id', 'Invalid free ID').notEmpty().isInt();
  ...
  const checkAuthorQuery = 'SELECT member_id FROM free_board WHERE num = ?';
  connection.query(checkAuthorQuery, [free_id], (err, result) => {
    if (err) {
      ...
      if (member_id !== author_id) {
        ...
        const deleteQuery = 'DELETE FROM free_board WHERE num = ?';
        connection.query(deleteQuery, [free_id], (err, result) => {
          if (err) {
            console.error('게시글 삭제 실패:', err);
            res.status(500).send('게시글 삭제에 실패했습니다.');
          } else {
...
// 댓글 삭제 라우트
router.post('/Free_board/comment/delete/:id', (req, res) => {
  const comment_id = req.params.id;
  const member_id = req.session.memberID;
  
  // 현재 사용자가 게시물의 작성자인지 확인
  const checkAuthorQuery = 'SELECT `member_id`, `free_id` FROM `members`.`comments_free` WHERE `comment_id` = ?';
  ...
  connection.query(checkAuthorQuery, [comment_id], (err, result) => {
    if (err) {
      ...
      const author_id = result[0].member_id;
      const free_id = result[0].free_id; // 게시글 ID도 가져옵니다.

      if (member_id !== author_id) {
        res.status(403).send('작성자가 아닌 사용자입니다. 댓글 삭제 권한이 없습니다.');
      } else {
        const deleteQuery = 'DELETE FROM `members`.`comments_free` WHERE `comment_id` = ?';
...
router.post('/comments', (req, res) => {
  ...
  const { comment: commentBody, free_id } = req.body;
  const member_id = req.session.memberID;
  const comment_date = new Date();
  console.log(`Session memberID: ${member_id}`);  // 세션 값 확인

  const query = 'INSERT INTO comments_free (content, member_id, review_id, comment_date) VALUES (?, ?, ?, ?)';
